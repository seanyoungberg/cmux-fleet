...
- **Never relocate a live agent across workspaces without reseating it afterward.** The agent's PROCESS keeps exporting the `CMUX_WORKSPACE_ID` it was launched with, and cmux resolves that agent's hooks by it, so after a cross-workspace move every hook silently no-ops against a workspace the agent no longer occupies. Only a reseat re-exports the env. The agent stays alive and works normally while its `Stop` never reaches the bus, so **the router silently drops every completion** (the conductor stalls with no wake), Feed gates go invisible, `updatedAt` freezes, and `fleet ls` shows it STALE though its pane is healthy. This state has a name — `detached`: present, but hook-dead — and `fleet doctor` now reports it. **Remedy: `fleet recycle <label>` after any `fleet move`, raw `cmux move-*`, or GUI drag of a live agent**, then confirm a `[QUEUE …]` line appears in `router.log` on its next `Stop`. `fleet move` does not reseat for you yet (that is agent-management v2 step 4, still gated). Better: launch into the target workspace up front (`CMUX_WORKSPACE_ID=<workspace-uuid> fleet launch …`). The Layout recipe's fold is safe because it moves a surface between panes of the *same* workspace, so the exported env never goes stale. (Earlier guidance blamed a desynced hook-store binding and a frozen `activeSessionsBySurface` pointer; the pointer was refuted as a signal — its write is path-dependent — and survives only as a forensic breadcrumb.)
