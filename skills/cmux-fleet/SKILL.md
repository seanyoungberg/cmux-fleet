...
- **Spawn + task a child:** `fleet launch <role> --parent $CMUX_SURFACE_ID` → `fleet drive-child <child-surface> "<task>"`
...
- **Moving a live agent across workspaces PERMANENTLY darkens it. There is no reseat that fixes it — only a fresh surface.** A cross-workspace move of a LIVE surface destroys that surface's agent-status registration inside the cmux app. The break is **surface-scoped and survives a process restart**, so `fleet recycle` — which re-execs the pane process on the *same* surface — comes back just as dark (and usually cannot even run: its quiet-gate reads the very lifecycle the break freezes). Root-caused, reproduced on a disposable seat, and live-verified 2026-07-10.
  - **Scope: observability only.** The agent still works, still receives its inbox, still completes turns, still answers `read-screen`/`send_text`. What dies is the stamping — `agentLifecycle` freezes, `fleet ls`/`vitals` read it STALE forever, cmux's sidebar drops its metadata. **Do not panic-wake a "detached" agent on this basis.**
  - **`fleet move` now REFUSES a live agent** (a warning printed next to completed damage is not a guard). Relocate with **`fleet move <label> --to-workspace <ws>|--own-workspace --archive-revive`**: it archives the agent and revives it onto a **fresh surface** in the target, resuming the **full session** (never the compact summary). Bystanders sharing the workspace are safe — the teardown downgrades to `close-surface` and keeps the workspace.
  - Better still: **launch children straight into their final workspace** and never move them.
  - Ground-truth check for a dark surface: count `sidebar.metadata.updated` events in `~/.cmuxterm/events.jsonl` keyed on `--panel=<SURFACE>`. In that payload **`--panel=` is the surface and `--tab=` is the workspace** — keying on `--tab=` inverts the answer.
