...
def test_ls_flags_dead_pid_running_ghost_stale(fs, monkeypatch, capsys):
    ...
    monkeypatch.setattr(fs, "lifecycle", lambda s: life.get(s, ""))
    monkeypatch.setattr(fs, "surface_has_live_pid", lambda s: pid.get(s, False))
    fleet.cmd_ls([])
...
def test_ls_pending_row_unbound_shows_pending_not_stale(fs, monkeypatch, capsys):
